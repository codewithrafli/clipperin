...
from clipper_cli.commands import (
    download_command,
    transcribe_command,
    analyze_command,
    chapters_command,
...
app = typer.Typer(
    name="clipper",
    help="Auto Clipper CLI - Create viral shorts from long videos",
    add_completion=True,
...
def main(
    ...
    """Auto Clipper CLI."""
    if version:
        typer.echo(f"clipper v{__version__}")
        raise typer.Exit()
...
def pipeline_command(
    ...
    """
    from clipper_core import Job, VideoDownloader, AudioTranscriber, ContentAnalyzer
    from clipper_core.ai import GroqClient
    from clipper_cli.config.settings import load_user_config
    ...
    with progress_bar(description="Running pipeline...", total=4) as prog:
        ...
        if use_ai:
            match config.ai.provider:
                ...
                case "gemini":
                    from clipper_core.ai import GeminiClient
                    ai_client = GeminiClient(api_key=config.ai.gemini_api_key)
                case "openai":
                    from clipper_core.ai import OpenAIClient
    ...
    console.print(f"  [cyan]clipper render[/cyan] {video_path.name} {chapters_path.name} -o {output_dir}")
